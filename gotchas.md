...
- The home feed `/index.xml` (what the nav "RSS" link + homepage autodiscovery point at) is built by `layouts/index.rss.xml` as a DENY-list: `where site.RegularPages "Type" "not in" params.nofeedSections`. It keys on Hugo **Type** (= the content dir name), so the posts section's Type is `post` (singular) even though `content/post/_index.md` sets `url: posts` — the URL is plural, the Type is not. So `nofeedSections` must name every non-post section (`notes` included) or that section leaks into the main feed, and a stale `posts` (plural) entry is a silent no-op. A 2026-03 "typo fix" flipped that dead `posts`→`post`, which finally matched the Type and silently emptied `/index.xml` of blog posts for six months (main feed became notes-only, so subscribers never saw posts). Section feeds `/posts/index.xml` and `/notes/index.xml` don't use nofeedSections — it's home-only. Separately: the home-feed item `<title>` must be bare `{{ .Title }}` (already escaped in RSS output); piping `| htmlEscape` double-encodes to `&amp;#39;`.
...
- Dropping in a new post from an `ia-writer://` link: the source lives in `~/Library/Mobile Documents/27N4MQEA55~pro~writer/Documents/Blog Posts/` (the `pro~writer` container, NOT the sibling `com~ia~writer` one, which is empty). Drafts have no frontmatter and no title — place at `content/post/<YYYY-MM-DD-slug>/index.md`, add frontmatter mirroring a recent post (date uses the `-06:00` offset, `generateSocialImage: true`, `translationKey`+`slug`; `bsky:` added later after cross-posting). Copy the body byte-for-byte with `cat` so curly quotes and fenced code blocks survive — don't retype it. Embed images with the `{{% figure src="name.png" caption="..." %}}` shortcode (auto WebP + responsive srcset), even terminal screenshots — the bare `![]()` in got-shells is the exception, not the rule. macOS screenshot filenames have a narrow no-break space (U+202F) before AM/PM, so a typed `cp` path fails; glob it instead (`~/Desktop/Screenshot*10.38.06*.png`).
- Co-editing race: Harper often has the post open in an external editor while an agent edits `index.md` via tools. His buffer goes stale the moment the agent writes, and his next save silently reverts the agent's edits (clobbered review fixes twice, 2026-09-22). Commit + push agent edits right away so the PR keeps them regardless of the working tree, then have Harper reload/close his buffer. Best: one writer per file at a time. Recover with `git checkout HEAD -- <file>` after confirming via `git diff HEAD -- <file>` that his save was a pure revert with no new content of his to preserve.
